...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    all_pos = [(i, j) for i in range(X_RANGE) for j in range(Y_RANGE)]
    variableDomainsDict[PAC]=all_pos
    variableDomainsDict[GHOST0]=all_pos
    variableDomainsDict[GHOST1]=all_pos

    max_obs = manhattanDistance((0, 0), (X_RANGE - 1, Y_RANGE - 1)) + MAX_NOISE
    obs_domain = list(range(0, max_obs + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elim_value in eliminationOrder:
            unjoinedFactor, joinedFactor = joinFactorsByVariable(currentFactorsList, elim_value)
            currentFactorsList = unjoinedFactor
            
            if len(joinedFactor.unconditionedVariables()) > 1:
                currentFactorsList.append(eliminate(joinedFactor, elim_value))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        total = self.total()
        randomValue = random.uniform(0, total)
        accumulatedProb = 0
        for key in self:
            if accumulatedProb + self[key] > randomValue:
                return key
            accumulatedProb += self[key]
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            #if the observed distance is None, the probability is 1, otherwise 0
            return 1 if noisyDistance is None else 0
        #if the observed distance is None and the ghost is not in jail
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            updatedBeliefs[ghostPosition] = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition) * currentBeliefs[ghostPosition]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for nextPos, prob in newPosDist.items():
                updatedBeliefs[nextPos] += currentBeliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        for posIndex in range(self.numParticles):
            self.particles.append(self.legalPositions[posIndex % len(self.legalPositions)])
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        essentially converts a list opython autograder.py -q q8 --no-graphicsf particles into a belief distribution.
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        particleWeights = DiscreteDistribution()
        
        for particle in self.particles:
            particleWeights[particle] += self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
        
        if particleWeights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [particleWeights.sample() for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #I used list comprehension instead of using 'newPosDist = self.getPositionDistribution(gameState, oldPos)'
