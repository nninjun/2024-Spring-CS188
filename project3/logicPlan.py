...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    # findModel(a)) = {A: True}
    # findModelUnderstandingCheck() = a.__dict__ is: {'op': 'A', 'args': ()}
    a = Expr('A')
    a.op='a'
    return({a: True})
    "*** BEGIN YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    clauses = []
    for lit in itertools.combinations(literals, 2):
        clauses.append(disjoin(~lit[0],~lit[1]))
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    if successorAxioms and t > 0:
        pacphysics_sentences.append(successorAxioms(t=t, walls_grid=walls_grid, non_outer_wall_coords=non_outer_wall_coords))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # We know wfich coords are walls:
    map_sent = [PropSymbolExpr(wall_str, x, y) for x, y in walls_list]
    KB.append(conjoin(map_sent))

    "*** BEGIN YOUR CODE HERE ***"
    KB.append(pacphysicsAxioms(t=0, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, walls_grid=walls_grid, successorAxioms=allLegalSuccessorAxioms))
    KB.append(pacphysicsAxioms(t=1, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, walls_grid=walls_grid, successorAxioms=allLegalSuccessorAxioms))
    
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0)) #Pacmain's current location (x0,y0)
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0)) #initial knowledge
    ...
    for t in range(50):
        ...
        #all possible pacman positions in non_wall_coords
        KB.append(conjoin([pacmanSuccessorAxiomSingle(x, y, time=t+1, walls_grid=walls_grid) for x, y in non_wall_coords]))
...
def foodLogicPlan(problem) -> List:
    ...
    for x, y in food:
        KB.append(PropSymbolExpr(food_str, x, y, time=0)) #True if there is a food at (x,y) at time t
        
    for t in range(50):   
        print("timestep :", t)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords]))

        goal_expr_list = [~PropSymbolExpr(food_str, x, y, time=t) for x, y in food] #all Food[x,y]_t are false
        ...
        #all possible pacman positions in non_wall_coords
        KB.append(conjoin([pacmanSuccessorAxiomSingle(x, y, time=t+1, walls_grid=walls) for x, y in non_wall_coords]))

        for x, y in food:
            #food has been eaten by Pacman
            KB.append(((PropSymbolExpr(pacman_str, x, y, time=t) & PropSymbolExpr(food_str, x, y, time=t)) >> ~PropSymbolExpr(food_str, x, y, time=t+1)))
            #food remains because Pacman didn't eat it
            KB.append((~PropSymbolExpr(pacman_str, x, y, time=t) & PropSymbolExpr(food_str, x, y, time=t)) >> PropSymbolExpr(food_str, x, y, time=t+1))
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        #all same with the code in question 6, but just change walls_grid to known_map.
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, sensorAxioms, allLegalSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))

        #also similar to the code in question 6
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        #similar to the code in question 7, 
        #instead of using sensorAxioms and fourBitPerceptRules, I used SLAMSensorAxioms and numAdjWallsPerceptRules
        ...
        #combine the condes in question 6 and 7.
        possible_locations = []
        for x, y in non_outer_wall_coords:
            pac_expr = PropSymbolExpr(pacman_str, x, y, time=t)
            wall_expr = PropSymbolExpr(wall_str, x, y)
...
def fourBitPerceptRules(t: int, percepts: List) -> Expr:
    ...
    for wall_expr, direction in zip(percepts, DIRECTIONS):
        percept_unit_clause = PropSymbolExpr(blocked_str_map[direction], time=t)
        if not wall_expr:
            percept_unit_clause = ~PropSymbolExpr(blocked_str_map[direction], time=t)
